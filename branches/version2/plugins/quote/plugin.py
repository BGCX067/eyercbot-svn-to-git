...
'''Quote script plugin. !quote add nick:nickname tags:tag1,tag2 quote:line to be quoted will add the quote to the user.  Tags are optional parameters for searching purposes. !quote random nick:nickname will pull a random quote for that user. !quote delete nick: nickname 1 will delete that number quote entry for the (optional) nick. !quote replace nick: nickname 1 New Quote will replace the quote at that entry for that nick.'''
...
def main(bot, user, target, msg):
	...
	if msg.split()[1].lower() == 'delete':
		deleteQuote(bot, user, target,  msg)
	
	if msg.split[1].lower() == 'replace':
		replaceQuote(bot, user, target, msg)
...
def deleteQuote(bot, usr, target, msg):
	'''
	Deletes quote.
	!quote delete nick:nickname #
	nick is optional, otherwise self is assumed.
	'''
	if 'nick:' in msg:
		msgList = msg.split()
		usr = msgList[msgLst.index('nick:') + 1]
		numIndex = msg.split()[msgLst.index('nick:') + 2]
	else:
		numIndex = msg.split()[2]
	
	if usr not in bot.users.database:
		bot.message(usr, target, 'I know no user by that name.')
		return
	
	user = bot.users.database[usr]
	
	try:
		quote = user.quotes.pop(int(numIndex))
		bot.message(usr, target, 'I removed the following quote: ' + quote + ' from ' + usr + '.')
		return
	except:
		bot.message(usr, target, 'I am unable to remove the quote for one reason or another.')
		return
	
def replaceQuote(bot, usr, target, msg):
	'''
	Replaces a quote string with a new (edited) string.
	All other variabels in the entry list remain intackt
	An edited by entry is added (if we ever convert to dict).
	'''
	if 'nick:' in msg:
		msgList = msg.split()
		usr = msgList[msgLst.index('nick:') + 1]
		numIndex = msg.split()[msgLst.index('nick:') + 2]
		newQuote = msg.split(' ', msgLst.index('nick:') + 3)[msgLst.index('nick:') + 3]
	else:
		numIndex = msg.split()[2]
		newQuote = msg.split(' ', 3)[3]
	...
	try:
		user.quotes[int(numIndex)][1] = newQuote
		bot.message(usr, target, 'I edited the following quote: ' + newQuote + ' for ' + usr + '.')
		return
	except:
		bot.message(usr, target, 'I am unable to edit the quote for one reason or another.')
		return
