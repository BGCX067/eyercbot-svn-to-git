'''Search plugin. !search google searchterm will search google with searchterm and return the first result. !search wikipedia searchterm will search wikipedia and return the first paragraph of the closest match. !search imdb movie title searches for movie title. !search noslang word will try to translate word.'''
...
sys.path.append('lib')
import imdb
import plugins.xrl.plugin as xrl
# This will prittyfy url
import urllib

import lib.html2irc as html2irc
import lib.webbot
...
def main(bot, user, target, msg):
	...
	if len(msg.split()) > 2:
		...
		# GOOGLE
		if msg.split()[1].upper() == 'GOOGLE':
			url = 'http://www.google.com/search?'
			webbot = urllib2.Request('%sq=%s'%(url, search_string.replace(' ','+')))
			webbot.add_header('User-Agent','')
			...
			#try:
			hits = re.search('Results <b>1</b> - <b>.*</b> of.*<b>(?P<hit>.+?)</b> for <b>', html)
			...
			entry = re.search('<li class=g(>| ).*?\x22.*?</div>',html)
			link =  re.search('(http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?', entry.group())
			test = re.search('class=l>(?P<title>.+?)</a>', entry.group())
			title = test.group('title')
			test = re.search('<div class="s">(?P<tag>.+?)<br>', entry.group())
			tag = test.group('tag')
			results = html2irc.html2irc(results + ' | ' + title + ': ' + link.group() + ' - ' + tag)
			...
			#	bot.message(user, target, message)
			#	print message
				
	
		# WIKIPEDIA
		if msg.split()[1].upper() == 'WIKI' or msg.split()[1].upper() == 'WIKIPEDIA':
			wikiUrl = 'http://en.wikipedia.org/w/api.php?format=yaml'
			wikiSearch = '&action=query&list=search&srwhat=text&srlimit=1&srsearch='
			wikiPage = '&action=parse&prop=text&page='
			mediawiki(bot, user, target, search_string, wikiUrl, wikiSearch, wikiPage)

	# IMDB
		if msg.split()[1].upper() == 'IMDB' and len(msg.split()) == 2:
			bot.message(user, target,  'Usage: !search imdb movie title searches for movie title')
			return None
		if msg.split()[1].upper() == 'IMDB':
			# We create an object which we interact IMDB with
			imdbBot = imdb.IMDb()
			movie_results = imdbBot.search_movie(search_string)
			movie = movie_results[0]
			imdbBot.update(movie)
			summary = movie.summary()
			# Removes some odd formatting
			summary_list = summary.replace('Movie\n=====\n',  '').split('\n')
			# Split output: 0: Title, 1: Genres.Director, 2: Writer, 3: Cast, 4: Runtime, 5: Country, 6: Language, 7: Rating, 8: summary
			output_string = summary_list[0] + ' ' + summary_list[1] + ' ' + summary_list[8]
			if len(output_string) > length:
				output_string = output_string[0:length]
			bot.message(user, target, output_string)
	
	# NOSLANG
		if msg.split()[1].lower() == 'noslang':
			url = 'http://www.noslang.com/search.php?st=' + search_string
			html = lib.webbot.bot(url)
			if "We didn't find any matches" in html:
				bot.message(user, target, 'No matches found.')
				return
			result = re.search('">(?P<result>.+?)<br>',html)
			message = html2irc.html2irc(result.group('result').replace('</abbr>', ''))
			bot.message(user, target, message)
			return

def mediawiki(bot, user, target,  searchString, wiki_url, wiki_search,  wiki_page):
	# TODO: search in their language
	# list=random will also be a fun feature
	redirect = ''
	# Wikipedia does not like odd characters'
	search_string = re.sub('<|>|[|]|{|}|\|', '', searchString)
	...
	if html['parse']['text']['text'].find('noarticletext') == -1 and html['parse']['text']['text'].find('REDIRECT') == -1:
		text_temp = re.search('<p>(?P<text>.+)</p>', html['parse']['text']['text'])
		text = text_temp.group('text')
		wiki_title=search_string
		results = html2irc.html2irc(wiki_title + ': ' + text).encode('utf-8')
	...
	if html['parse']['text']['text'].find('REDIRECT') != -1:
		wiki_title = re.search('<li>(REDIRECT:)|(REDIRECT) <a .+>(?P<title>.+)</a>', html['parse']['text']['text']).group('title')
		...
		results = html2irc.html2irc(wiki_title + redirect +': ' + text).encode('utf-8')
		# Trims results down to length
		if len(results) > length:
			results = results[0:length]
		bot.message(user, target, results)
	...
	if wiki_search_results['query']['search'] == None:
		bot.message(user, target, 'No matches found for your search')
		return None		
